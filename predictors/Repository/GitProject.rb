...
class GitProject
		...
		File.open(localClone+pathInput+prefixProjectName+"_MergeScenarioList.csv", "r") do |text|
			...
			text.each_line do |line|
				...
				if !countLeftCommits.eql? "0" and  !countRightCommits.eql? "0" #pra que esse if?? nao faz sentido!!!!
					...
					arithmeticMeanMergeCommits = (Float(Integer(countLeftCommits) + Integer(countRightCommits))/2).round(2)
		...
		File.open(localClone+pathOutput+prefixProjectName+"_CommitList.csv", 'r') do |f1| 
		 while line = f1.gets 
		    ...
		    rightAuthorsList = []
				...
				arithmeticMeanMergeAuthors = (Float(Integer(leftAuthorsList.uniq.length) + Integer(rightAuthorsList.uniq.length))/2).round(2)
			...
			while line = f1.gets
				f1.each_line do |line|
					...
					if leftDate > rightDate
						contributionConclusionDelay = (leftDate - rightDate).to_i
					elsif rightDate > leftDate
						contributionConclusionDelay = (rightDate - leftDate).to_i
					else
						contributionConclusionDelay = 0
					end
		...
		File.open(localClone+pathOutput+prefixProjectName+"_CommitList.csv", 'r') do |f1|
		 while line = f1.gets 
	      ...
	      rightAuthorsList = []
				...
				arithmeticMeanLifetime = (Float(leftLifetime + rightLifetime)/2).round(2)
  ...
  # It can be used to evaluate individual projects that compose the aggregated sample.

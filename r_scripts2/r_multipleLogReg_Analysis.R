...
multipleGLM <- function(fileName, responseVariable, predictorsVariablesList, selectModel, usesVIF) { 
  ...
  if (length(predictorsNameList)>=1){
    ...
    print(paste("Predictor, Estimate, Pr(>|z|)")) #cabeçalho
    ...
    if(length(significantPredictors_5)>= 1 | length(significantPredictors_10)>=1){
      ...
      if (length(significantPredictors_5)>=1){
        ...
        for (i in 1:length(significantPredictors_5)) { #apenas significativos
          ...
          #para imprimir em relatório texto livre
  ...
  #Resumo para imprimir no reltório por projeto 
...
##############################################################################
##Script para rodar N amostras de uma só vez e resumo consolidado
#################################################################################
...
response <- "isConflicting"
#predictorsList <- c("existsCommonSlice")
#predictorsList <- c("existsCommonSlice", "numberOfCommitsGeoAverage")
#predictorsList <- c("existsCommonSlice","numberOfAuthorsGeoAverage", "numberOfChangedFilesGeoAverage")
#predictorsList <- c("existsCommonSlice", "numberOfAuthorsGeoAverage","numberOfChangedLinesGeoAverage")
#predictorsList <- c("existsCommonSlice", "numberOfCommitsGeoAverage","contributionConclusionDelay")
#predictorsList <- c("existsCommonSlice","numberOfAuthorsGeoAverage","numberOfChangedFilesGeoAverage","minimumLifeTimeGeoAverage", "contributionConclusionDelay")
predictorsList <- c("existsCommonSlice","numberOfAuthorsGeoAverage","numberOfChangedLinesGeoAverage","minimumLifeTimeGeoAverage", "contributionConclusionDelay")
